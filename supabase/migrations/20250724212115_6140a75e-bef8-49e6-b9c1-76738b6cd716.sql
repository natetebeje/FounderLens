-- Fix for validation workflow loading error - atomic workflow creation
-- This function handles the "get or create" workflow logic atomically to prevent race conditions
...
BEGIN
  ...
  WHERE validation_workflows.opportunity_id = p_opportunity_id;
  ...
  -- If no workflow exists, create one using ON CONFLICT to handle race conditions
  ...
  -- Now get the workflow (either the one we just created or one created by concurrent request)
  SELECT * INTO workflow_record
  FROM public.validation_workflows
  WHERE validation_workflows.opportunity_id = p_opportunity_id;
